// ══════════════════════════════════════════════════════════════
//  Capture — только медиа (скрины, фото, видео)
// ══════════════════════════════════════════════════════════════

const CAP_KEY = "cortex_caps_v2";
let captures = [];

function loadCaps() {
  try { captures = JSON.parse(localStorage.getItem(CAP_KEY) || "[]"); }
  catch { captures = []; }
  renderCaps();
}
...
function clearCaptures() {
  if (!captures.length) { toast("ℹ️","Галерея пуста",2000); return; }
  openFsOverlay(`
    <div class="fs-title">🗑 Очистить?</div>
...
function doClear() {
  captures = []; saveCaps(); renderCaps(); closeFsOverlay();
  ...
  if (c.mediaType === "image" && c.b64) {
    openFsOverlay(`
      ...
      <div class="fs-actions">
        ...
        <button class="btn ok" onclick="dlCap('${c.b64}','${c.filename || "photo.jpg"}','${c.mime}')">📥 Скачать</button>
      </div>
    `);
  } else if (c.mediaType === "video_file" && c.url) {
    openFsOverlay(`
...
function openRecordPicker(type) {
  ...
  openFsOverlay(`
    ...
    <div class="fs-desc">Длительность</div>
    <div class="grid g3">
